...
<div class="w3-row">
  <!-- CATEGORIAS -->
   <div class="card-categorias w3-container w3-row w3-card-2 w3-white w3-round w3-margin-bottom w3-padding-16">
      <div class="w3-row">
        <div class="w3-col s6"><h2><?=$forum->getNomeForum()?></h2></div>
      ...
      <?php if(empty($listaTopico)){
          print "<p><h4>Este forum ainda não possui Topicos.</h4></p>";
        }else{ ?>
              ...
              <div class="">  
                ...
                <a class="w3-button w3-block w3-left-align" href="index.php?req=controller_topicos&acao=listar&idTopico=<?=$topico->getIdTopico()?>">
                  <div class="w3-row" style="height: 50px; ">
                     <div class="nome-topico w3-col s6" style="height: 100%;"><?=$topico->getTitulo()?></div>
                      <div class="posts-topico w3-col s3"><?=$topico->getQtdPosts()?> POSTS</div>
                      <div class="user-topico w3-col s3"><img src="img/fotos-perfil/<?=$topico->getFotoUsuario()?>" alt="Foto Usuario" class="foto-perfil-mini w3-margin-right"><?=$topico->getNomeUsuario()?> - <?=date("d/m/Y",strtotime($topico->getDataCriacao()))?></div>
                  </div>          
                </a>
                <?php }?>
              </div>
...
<div id="modal-topico" class="w3-modal">
    <div class="w3-modal-content w3-card-4 w3-animate-zoom" style="max-width:75vw;">
      ...
      <form class="w3-container" action="index.php?req=controller_forums&acao=cadastrar&idUsuario=<?=$_SESSION['id_usuario']?>&idForum=<?=$_GET['idForum']?>" method="post">
